(function () {
    ...
    var currentStep = 1;
    var participantsInput = document.getElementById('participants-input');
    ...
    function getParticipants() {
        var n = parseInt(participantsInput.value, 10);
        if (isNaN(n) || n < 1) {
            return config.minParticipants || 1;
        }
        if (config.maxParticipants && n > config.maxParticipants) {
    ...
    function updateTotals() {
        var total = (config.unitPrice || 0) * getParticipants();
        var label = formatMoney(total);
        if (summaryTotal) {
            summaryTotal.textContent = label;
        }
        if (payAmountLabel) {
            payAmountLabel.textContent = label;
        }
    }
    ...
    if (participantsInput) {
        participantsInput.addEventListener('input', updateTotals);
        participantsInput.addEventListener('change', updateTotals);
    }
    ...
    form.addEventListener('submit', function (ev) {
        ...
        fetch(config.submitUrl, {
            ...
            .then(function (result) {
                ...
                showError((result.data && result.data.message) || 'تعذّر إتمام الحجز.');
            })
            .catch(function () {
                if (bookingLoading) {
                    bookingLoading.hidden = true;
